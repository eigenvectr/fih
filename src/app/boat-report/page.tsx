...
const FEATURES: Detail[] = [
  ...
  },
  {
    ...
    body: [
      "Aluminum frame (laughs at salt ramps), side guide-ons that funnel the boat home in wind, and a spare tire. Tie-down lesson (Sep 2026): the transom overhangs the rear crossmember, so 43\" retractable transom straps (the Rhinos) can't reach a legal down-pull point — the answer is 2\" × 16' GUNWALE straps over the rear deck. The trailer's existing pair ARE BoatBuckle Pro Series — but sun-rotted and slipping through the Kwik-Lok (UV-glazed webbing won't bite): retired. Replacement (decided Sep 2026): two fresh BoatBuckle Pro Series Kwik-Lok F17636 gunwale straps, sold one per box — two for redundancy (a single popped buckle can't free the stern) and to spread the load so neither strap needs crushing tension on an aluminum rub rail. Kwik-Lok technique: full hand-tension on the tail BEFORE flipping the lever, lever snapped fully flat, tail in the closure, recheck at five miles.",
    ],
    how: [
      ...
      "Gunwale vs transom straps, decided by geometry: a transom strap must pull down within ~30° of vertical — if the stern eyes sit far aft of the last anchor point, the strap drags the transom forward and stresses the hull corner. First check the boat isn't simply sitting too far back (bunks should reach the transom; the winch stand/bow stop on the tongue is adjustable, and moving the boat forward also fixes tongue weight). If the frame ends early, anchor at the bunk ends (through-bolted eye or CE Smith bracket, never the bare wood) and run BoatBuckle Pro Series 6' transom straps (F17633, pads included). Only if no anchor gets within 30° do gunwale straps become the tool.",
